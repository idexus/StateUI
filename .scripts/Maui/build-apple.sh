...
# sources sit under .build/checkouts/StateUI/lib/StateUI/Sources - inside a
...
# NOTE -D MAUI.
#
# Every Swift module the MAUI host compiles - the library and the application
# alike, on every platform - is compiled with the MAUI condition, so Swift code
# written for this host alone stands under `#if MAUI`. Android and Linux pass it
# to SwiftPM as -Xswiftc -DMAUI.
#
