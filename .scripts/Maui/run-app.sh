...
case "$PLATFORM" in
  ...
  ios)
    ...
    #
    # With none booted, this boots one itself: Simulator.app opened on its own
    # boots nothing when its last device was shut down, and `bootstatus booted`
    # then fails at once with "No devices are booted". The newest iPhone the
    # installed runtimes offer is the one.
    if ! xcrun simctl list devices booted | grep -q "Booted"; then
      DEVICE="$(xcrun simctl list devices available | grep -E '^[[:space:]]+iPhone' | tail -n 1 \
        | grep -oE '[0-9A-F]{8}(-[0-9A-F]{4}){3}-[0-9A-F]{12}')"
      if [[ -z "$DEVICE" ]]; then
        echo "ERROR: no iPhone simulator is installed - add one in Xcode, Window > Devices and Simulators."
        exit 1
      fi
      echo "No simulator is booted. Booting $DEVICE..."
