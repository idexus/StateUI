...
final class DrivenWireTests: XCTestCase {
    ...
    func testEveryDrivenModifierIsWrittenDown() throws {
        let number = State(wrappedValue: AnimatedValue(0.5))
        let colour = State(wrappedValue: AnimatedValue(Color("#102030")))
        let inset = State(wrappedValue: AnimatedValue(Thickness(4)))
    ...
    func testADrivenInputIsWrittenDown() throws {
        ...
        let steps = State(wrappedValue: AnimatedValue(3.0))
    ...
    func testTwoDrivenPropertiesOnOneElementNumberInTheOrderTheirNamesDo() {
        let moved = State(wrappedValue: AnimatedValue(0.0))
        let faded = State(wrappedValue: AnimatedValue(1.0))
        let differ = Differ()
    ...
    /// `Slider().value($level)` says - over an `AnimatedValue`, and over a
