...
//
// An `AnimatedValue` is the one shape a `@Hosted` carries that says more than
...
// are HostedTests' business; this file is about the journey itself.
...
final class JourneyTests: XCTestCase {
    ...
    func testTheJourneysLanesReadAsValues() {
        let rotation = Hosted(wrappedValue: AnimatedValue(2.0))
    ...
    func testWritingTheValueOnTheBindingSnapsIt() {
        let rotation = Hosted(wrappedValue: AnimatedValue(0.0))
    ...
    func testSnappingSaysHereGoingNowhereAndStandingStill() {
        let box = Hosted(wrappedValue: AnimatedValue(0.0))
    ...
    func testALawStatedAtTheValueIsOnTheImageFromBirth() {
        let plain = Hosted(wrappedValue: AnimatedValue(0.0))
        let stated = Hosted(wrappedValue: AnimatedValue(0.0, motion: .spring()))
