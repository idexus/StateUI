...
//
// An `AnimatedValue` is the one shape a `@State` carries that says more than
...
// are CarriedStateTests' business; this file is about the journey itself.
...
final class JourneyTests: XCTestCase {
    ...
    func testTheJourneysLanesReadAsValues() {
        let rotation = State(wrappedValue: AnimatedValue(2.0))
    ...
    func testWritingTheValueOnTheBindingSnapsIt() {
        let rotation = State(wrappedValue: AnimatedValue(0.0))
    ...
    func testSnappingSaysHereGoingNowhereAndStandingStill() {
        let box = State(wrappedValue: AnimatedValue(0.0))
    ...
    func testALawStatedAtTheValueIsOnTheImageFromBirth() {
        let plain = State(wrappedValue: AnimatedValue(0.0))
        let stated = State(wrappedValue: AnimatedValue(0.0, motion: .spring()))
