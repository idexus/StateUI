...
extension State where Value: PersistentValue {
    ...
    /// this is, the wrapper's own name says - `@State`, `@Working` - and the
    /// brackets say only what ELSE is true of one: when it asks, and where it
    /// is kept. A key is not a kind: a kept state IS a described one, with
    /// somewhere to be written down as well. And the UNLABELLED position on this wrapper
