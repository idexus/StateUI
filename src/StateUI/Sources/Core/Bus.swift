...
//
//   the BUS   a `@Bus`. The host owns what it currently is and pushes it in;
//             nothing here asks for a render when it moves.
//   the RULE  the author's arithmetic, in Swift, which the host CALLS when a
//             bus moves and gets numbers back - where each child of a layout
...
// for `PlacedLayout`, reading the buses by their own names - reading one
...
/// A value that can ride a bus - what it is as the number that crosses, and
/// back. This library's own.
///
/// The boundary carries numbers, so whatever a bus holds says how it is one:
/// a `Double` is itself, an `Int` is the nearest whole one, a `Bool` is nought
/// or one. A type of the application's own joins by saying the same.
public protocol BusValue {
...
/// What a bus's value IS, across every render.
///
...
/// the bus number is issued against. It holds the CROSSING rather than the
...
final class BusStorage: @unchecked Sendable, NamedState {
    ...
    nonisolated(unsafe) var bus: Int32?
...
/// A bus, whatever value rides it - what a layout FOLLOWS, however each of
...
/// nothing tells the tree the value moved. What a bus is for is arithmetic
...
///
/// What rides it is any `BusValue` - a `Double` today for an offset and a
...
public final class Bus<Value: BusValue>: HostBus, @unchecked Sendable {
    ...
    public init(wrappedValue: Value) {
        super.init(BusStorage(wrappedValue.crossing))
    }
    ...
    /// What `$scrolled` gives: the bus itself, for a scroller to report
