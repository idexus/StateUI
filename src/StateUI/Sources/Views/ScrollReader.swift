...
public struct ScrollReader: ContentView {
    ...
    private var reportsX: Link<Double>?
    private var reportsY: Link<Double>?
    ...
    public func scrollX(_ value: Link<Double>) -> ScrollReader {
        var copy = self
        copy.reportsX = value
        return copy
    }
    ...
    /// - Parameter value: the driven state it is written into.
    /// - Returns: the reader, reporting there.
    public func scrollY(_ value: Link<Double>) -> ScrollReader {
        var copy = self
        copy.reportsY = value
        return copy
    }
...
extension ScrollView {
    ...
    func reporting(x: Link<Double>?, y: Link<Double>?) -> ScrollView {
        var scroller = self
