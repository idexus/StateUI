...
`priority` supplies ordering information while `order` distinguishes primary
from secondary actions. The host chooses the native placement appropriate to
the window and available space. Give stable identities to items whose list can
change.
