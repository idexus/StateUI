...
Declared in `lib/StateUI/Sources/Contracts/Elements/TitleBarContract.swift`.
...
| `background` | property | `Background` | native | ✅ | ☑️ |  |  |  |  |  | AppKit paints a colour on the window's bar band; a brush is drawn only by `Border`. |
| `focus` | act | `() -> Bool` |  | ✅ |  |  |  |  |  |  |  |
