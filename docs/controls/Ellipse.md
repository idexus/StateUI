...
Declared in `lib/StateUI/Sources/Views/Ellipse.swift`.

## Ellipse's own members

Ellipse declares no members of its own; everything it takes comes from the sections below.
...
| `stroke` | property | ✅ | ✅* |  |  |  |  |  | AppKit strokes with a colour; a gradient brush draws no outline. |
