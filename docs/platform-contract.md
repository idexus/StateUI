...
| `TabbedView` | `NSTabView`: tabless under a select-one `NSSegmentedControl` in the window's `NSToolbar`, with top tabs where no window toolbar serves it | `UITabBarController` | `GtkStack` + `GtkStackSwitcher`; libadwaita `AdwViewStack` | Material Components `BottomNavigationView` (?) | `NavigationView` with a top pane | ARIA `tablist` |
