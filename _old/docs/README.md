...
Write deterministic, reactive native interfaces in Swift. StateUI owns the
description, identity, state journeys and arithmetic; a platform host
materializes that description as AppKit, UIKit, Android Views, WinUI or GTK
controls. The existing MAUI host remains the compatibility path while those
independent native hosts are proved one platform at a time.
...
*The gallery is one Swift tree rendered as native controls. The screenshots
show the compatibility host on Windows, Android, Mac Catalyst, iPhone and GTK;
the same Gallery module now also runs directly through AppKit with no .NET
process.*
...
- **Getting started**
  ...
  - [Running Gallery with AppKit](#running-gallery-with-appkit)
  - [Your first page, line by line](#your-first-page-line-by-line)
  - [StateUI's control vocabulary](#stateuis-control-vocabulary)
...
- **The guide**
  ...
  - [Layout](#layout)
  - [Grid](#grid)
  - [Authored and derived layouts](#authored-and-derived-layouts)
...
ContentPage, VerticalStackLayout, Label, Button, Image, BoxView and Entry are
materialized as AppKit objects; an unsupported control remains visible as a
diagnostic label. Entry uses AppKit's own text editor and reports through the
same host-carried state that every backend shares. The regular HelloWorld MAUI
targets remain unchanged.

### Running Gallery with AppKit

The complete Gallery source module also runs in a native macOS application:

```bash
.scripts/build-gallery-appkit.sh debug
open apps/Gallery/.build/debug/GalleryAppKit.app
```

In VS Code, choose **Debug Gallery (AppKit)** and press **F5**. The build creates
a signed application bundle containing its dynamic libraries, image resources
and the generated multi-resolution StateUI icon. The first native slice covers
the complete `Application → Scene → Window` ownership path: *File ▸ New Window*
opens another scene, a scene owns its main and auxiliary windows, focus moves
the three session phases without cross-scene leakage, and closing the last
window leaves the process ready for a Dock reopen. AppKit restoration preserves
every scene's identity, frame, auxiliary windows and `@State(sceneKey:)` values;
auxiliary windows may be delivered before their main window and are still
claimed deterministically. Application `@State(persistentKey:)` values use
`UserDefaults` when `.preferences` is selected.

The same host also covers the application and page shells, `Grid`, both stacks, `AbsoluteLayout`,
`ScrollView`, `Border`, `Label`, `Button`, `Image`, `BoxView`, `Entry` and
`Slider`, plus native tap recognition, typed state-driven properties, frame
feeds and engine-authored placements. One
display-clocked motion channel belongs to each state number: every control that
wears it sees the same eased or spring value, a retarget keeps its velocity, and
the host reports truthful frames and exact landings back to the core. Controls
not yet implemented by this host remain visible as diagnostic labels.
...
- **`@State` is what the page remembers between renders.** `count` survives
  ...
  difference - the button's caption - is sent to the native `Button` on the
  screen. Nothing else on the page is touched.
- **A modifier names one StateUI capability.** `.fontSize(28)`, `.spacing(16)`,
  `.padding(30)` and `.horizontalOptions(.center)` mean the same thing at every
  host boundary. Platform APIs are evidence for that common vocabulary, not
  its owner.
- **A handler is a closure, and it may `await`.** `.onClicked { count += 1 }`
  runs on the thread the host draws on, through the library's own `@MainThread`
  ...
  StateUI process on Android or Windows.
...
## StateUI's control vocabulary

StateUI uses one name for a capability all supported native control families
share. Existing names remain where they state that meaning honestly; they are
changed or removed when evidence across AppKit, UIKit, Android Views, WinUI,
GTK and the DOM says they do not.

The built-in vocabulary has one checked ownership contract in
`Core/HostContract.swift`. It classifies all 66 built-in node types and all 182
property names, and a source-reading test fails when a new token has no owner:

| Owner | Controls and properties mean |
|---|---|
| **native** | every base host implements the same observable capability with its toolkit |
| **adaptive** | every host follows its platform's shell convention; where a platform has no counterpart, the neutral behavior is part of the modifier's documentation |
| **StateUI** | one Swift composition derives the behavior from smaller primitives before the settled host boundary |
| **structure** | the value carries tree, layout, state-channel or session protocol data rather than configuring a visual object |
| **provider** | an optional package owns the capability; it never becomes a silent requirement of every base host |

The small native visual set is deliberate: text, image, buttons, text input,
choice, ranges, progress, date and time, web content, scrolling, linear and
authored placement, drawing surfaces, and platform application chrome. Shells
such as navigation, tabs and flyout are adaptive native containers while Swift
continues to own their state and identity. `FlexLayout`, `Grid`, shapes,
`ImageButton`, indicators, refresh and row swipe behavior are classified as
StateUI compositions; `Map` and `Pin` are provider surface. Compatibility hosts
may still receive one of those nodes while its vertical migration is being
completed, but that temporary route is not the target contract.
...
- **A modifier exists on the narrowest StateUI capability tier that owns it.**
  `.spacing()` belongs to a stack, `.placeholder()` to text input and
  `.opacity()` to every visual element. A host maps those tiers onto the
  nearest native control concepts without exposing a toolkit inheritance
  hierarchy.
- **Two abbreviations, and no others.** `VStack` and `HStack` stand in for
  ...
  An `async` call carries no redundant suffix - `evaluateJavaScript`, not
...
The StateUI documentation and Gallery are the public reference. Platform
documentation remains useful when evaluating whether a capability has an honest
native implementation across AppKit, UIKit, Android Views, WinUI and GTK.

Every modifier, initializer and enum case carries a doc comment stating its
StateUI meaning. During the native-host migration, comments that merely name a
compatibility-host property are rewritten around that portable meaning:
...
## The application, its scenes, windows and pages

This is StateUI's stable application model, shared by every host: an
`Application` declares its `Scene`, a scene owns its windows, a `Window` shows
a page, and that page is either a screenful of content or an ARRANGEMENT of
other pages - a stack, a set of tabs, a menu beside a detail. A window alone is
a scene of one window, and a session that opens more windows than one is written
out in [More than one window](#more-than-one-window).
...
Each backend materializes that description as its real platform controls.
Which tab is showing is a value of the application's
...
Window geometry and capabilities are the window session's state: the same
StateUI names are written from any view in the window, and a supporting native
host follows when they change:
...
or `.destroying`: native lifecycle events exposed as StateUI state. Where the
APPLICATION stands is `application.phase` on the `ApplicationSession`, and
where one workspace or document session stands is `scene.phase` - see
...
rather than going under it - while the application is in front. AppKit maps
these to its native window ordering and visibility. The Window menu lists the
scenes by their main windows, never a window beside one.
...
**The platform opens sessions too**: *File ▸ New Window* in the AppKit host,
the window controls on an iPad, and the Dock icon on a Mac that kept the process
alive after its last window closed. On a Mac and an iPad the system RESTORES the
...
**Where a second window exists**: native AppKit, iPad, Mac Catalyst and Windows. A phone has
...
## Authored and derived layouts

`AbsoluteLayout` is the smallest host-facing placement surface: it asks each
child for an authored rectangle and maps directly to native view coordinates.
...
`AbsoluteLayout.autoSize` asks the child for its native measured size, so only
its position is dictated. The attached names retain the `absoluteLayout` prefix
because the parent layout is the owner of their meaning.

`FlexLayout` and `Grid` are not settled host primitives. No common native
arbitrary-child control supplies the full flex or grid contract on all target
families, and item collection layouts are not substitutes for general child
layout. The existing API remains available through compatibility renderers
during migration. Its destination is one StateUI implementation over
measurement and authored placement, after which those nodes and their attached
properties no longer cross the host boundary.
...
**`LazyList` is the public list contract here.** StateUI owns its logical item
order, stable identities, changes and row subtrees. A direct host maps that
contract to its native virtualized items surface: `NSCollectionView` (or
`NSTableView` for a strict one-column AppKit list), `UICollectionView`, Android
`RecyclerView`, WinUI `ItemsView`, GTK `ListView`/`GridView`, or the DOM. The
native control owns the viewport, cells, reuse, input and accessibility.

The compatibility implementation still describes a visible row window over a
scroller while the explicit collection payload is added to `HostPatch` and
Wire. A direct host must not put that already-windowed subset inside another
virtualizer: that would create two item models, two reuse policies and two
scroll anchors.
...
| | Native collection surface | StateUI collection contract |
| --- | --- | --- |
| Item source and identity | receives stable logical items | owned by Swift |
| Viewport and cells | owned by the platform control | requested by item identity |
| Reuse | native cell lifecycle | a rebinding replaces the complete row subtree contract |
| Selection and scroll | native interaction | reported into the same StateUI state |
...
The sample app is a gallery: a page per group lists what is in it, and a page
per sample shows the control, what it is for and the Swift that produced it.
During direct-host bring-up the root is a deterministic, scrollable
`GroupListPage`; the richer card-based `HomePage` remains the final Gallery
acceptance surface after the native collection and derived placement contracts
are complete.
...
│   ├── GroupListPage.swift the temporary direct-host list of every group
...
which one has it. A group's name also has to fit the compact navigation surface
that carries it. What goes in each follows StateUI's contract and the common
...
The source of truth is StateUI's checked host contract, not one toolkit's
catalog. The base set stays intentionally small so each renderer remains an
adapter around platform controls:

| Ownership | Current built-in surface |
|---|---|
| **native primitives** | Label, Button, Entry, Editor, SearchBar, Picker, DatePicker, TimePicker, Switch, Slider, Stepper, ActivityIndicator, ProgressBar, Image, BoxView, Border, VerticalStackLayout, HorizontalStackLayout, AbsoluteLayout, Overlay, ScrollView, WebView, GraphicsView |
| **adaptive native shell** | ContentPage, NavigationPage, TabbedPage, FlyoutPage, TitleBar |
| **StateUI composition** | CheckBox, RadioButton, ImageButton, IndicatorView, RefreshView, SwipeView, Grid, FlexLayout and the shape family; compatibility hosts keep their current paths until each vertical replacement is proved |
| **optional provider** | Map and Pin |
| **protocol structure** | Application, Scene, Window, slots, formatted text, menus, toolbars, modal state, visual-state data and the internal composed placeholder |

The next primitive is a semantic native collection boundary for `LazyList` and
later grid/card compositions. It is not named after any platform class. StateUI
owns item identity and changes; the toolkit owns cells, viewport and reuse.

#### The properties, and the families deliberately left out

Every built-in property is classified beside the controls. A native property
is a required observable capability, an adaptive property documents its neutral
fallback, a StateUI property is consumed by composition, structural data has a
protocol owner, and provider data never enters the base-host requirement. The
test rejects both an unclassified property and a contract entry with no token.

Four implementation families are absent by design, and none is an oversight:
...
The current API includes control-specific modifiers such as a Switch's `offColor`, an
