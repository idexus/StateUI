...
extension WinUIElement {
    ...
    func configureGestures() {
        ...
        var hearing: UInt32 = 0
        if element.handler(.tapped) != nil { hearing |= UInt32(StateUIHearingTaps.rawValue) }
        if Self.pointerEvents.contains(where: { element.handler($0) != nil }) {
            hearing |= UInt32(StateUIHearingPointer.rawValue)
        }
        ...
        if drags, (value(.panTouchCount)?.number ?? 1) == 1 { hearing |= UInt32(StateUIHearingDrags.rawValue) }
        if element.handler(.pinchUpdated) != nil { hearing |= UInt32(StateUIHearingPinches.rawValue) }
    ...
    private func heard(_ heard: WinUIHeard) {
        switch heard {
        case .tap(let run):
