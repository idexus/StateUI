...
final class WinUIImageViewTests: XCTestCase {
    ...
    func testAnSVGAskedForByItsPNGNameIsDrawnAtItsOwnSize() throws {
        try onUIThread {
            ...
            let image = try XCTUnwrap(host.views(WinUIImageView.self).first)
    ...
    func testAMissingPictureIsSaidMissing() throws {
        try onUIThread {
            let host = WinUIRenderer.running { VStack { Image("nowhere.png") } }

            XCTAssertEqual(try XCTUnwrap(host.views(WinUIImageView.self).first).found, false)
        }
    }
