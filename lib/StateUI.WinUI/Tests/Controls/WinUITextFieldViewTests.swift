...
final class WinUITextFieldViewTests: XCTestCase {
    ...
    func testEachKeystrokeReachesTheStateAndStaysTyped() throws {
        try onUIThread {
            ...
            let host = WinUIRenderer.running { greeting(name: name) }
            ...
            XCTAssertEqual(host.views(WinUILabelView.self).map(\.text), ["Hello, Ada!"])
        }
    }
    ...
    func testTypingStopsAtTheMaximumLength() throws {
        try onUIThread {
            let name = State(wrappedValue: "")
            let host = WinUIRenderer.running { greeting(name: name, maximumLength: 3) }
            let field = try XCTUnwrap(host.views(WinUITextFieldView.self).first)
