...
// thing and are not. Both make a class report its writes, and they report to
// different listeners. A `@State` calls this library's renderer, and the render
// that follows rebuilds the views that read the property. `@Observable` hands its
...
//
// A model is one or the other, and the one this library hears is the class
// whose properties are `@State`.
//
...
//
// A model somebody else declared `@Observable`, whose properties cannot be
// given `@State` from outside the package that declares them, is bridged in
// the application: read it inside `withObservationTracking` and call
