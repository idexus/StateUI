...
final class HostRenderTests: XCTestCase {
    ...
    func testANativeHostConnectsPlatformScenesBeforeRenderingThem() {
        ...
        StateUIHost.connectScene(restoring: ["shade": .string("dusk")])
        StateUIHost.connectScene(restoring: [:])
    ...
    func testANativeHostDecodesAndEncodesAJourneyWithoutItsLaneLayout() throws {
        ...
        let decoded = try XCTUnwrap(StateUIHost.journey(from: StateUIHost.value(of: described)))

        XCTAssertEqual(decoded, described)
    }

    func testANativeHostRejectsMalformedJourneyBookkeeping() {
        XCTAssertNil(StateUIHost.journey(from: .lanes([
            0, 1, 0,
            0, 0, 0,
            0,
            .infinity,
    ...
    func testANativeHostReportsOnlyTheJourneyGroupsItWalked() throws {
        ...
        let before = try XCTUnwrap(StateUIHost.value(for: binding))
        let journey = try XCTUnwrap(StateUIHost.journey(from: before))
        ...
        XCTAssertTrue(StateUIHost.report(frame, updating: .frame, through: binding))
        ...
        XCTAssertTrue(StateUIHost.report(frame, updating: .position, through: binding))
