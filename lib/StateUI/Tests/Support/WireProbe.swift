...
public enum WireProbe {
    ...
    /// One value's line, spelled as `PatchDump` spells it - one table of
    /// vocabularies for the typed dump and this one.
    private static func line(for value: PropValue, under key: String? = nil) -> String {
        PatchDump.line(for: value, under: key)
    }
    ...
    private static func spelled<Vocabulary: RawRepresentable>(
        ...
        PatchDump.spelled(member, as: vocabulary)
    }
