...
enum SourceTree {
    /// `lib/StateUI.Host/Sources`, the host layer's.
    static var hostSources: URL {
        repository.appendingPathComponent("lib/StateUI.Host/Sources")
    }
    ...
    private static func readAllSources() throws -> [(path: String, text: String)] {
        ...
        // The core's sources under their own paths; the host layer's under its module's name, `StateUIHost/`.
        for (prefix, root) in [("", sources), ("StateUIHost/", hostSources)] {
    ...
    static func testSources() throws -> [(path: String, text: String)] {
        ...
        let targets = [
            ("StateUITests", repository.appendingPathComponent("lib/StateUI/Tests")),
