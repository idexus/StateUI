...
internal static class PropertyTable
{
    ...
    /// property's name, so the accessors in <see cref="Values"/> are reached
    ...
    private static BindableProperty? Own(HostNodeType targetType, HostProp name, BindableObject? target)
    {
        return targetType switch
        {
            ...
            {
                HostProp.CornerRadius => TransformedRoundRectangle.CornerRadiusProperty,
            ...
            {
                HostProp.X1 => TransformedLine.X1Property,
                HostProp.Y1 => TransformedLine.Y1Property,
                HostProp.X2 => TransformedLine.X2Property,
                HostProp.Y2 => TransformedLine.Y2Property,
                _ => ShapeProperty(name),
            },

            HostNodeType.Path => name switch
            {
                HostProp.Data => TransformedPath.DataProperty,
                _ => ShapeProperty(name),
            },

            HostNodeType.Polygon => name switch
            {
                HostProp.Points => TransformedPolygon.PointsProperty,
                HostProp.FillRule => TransformedPolygon.FillRuleProperty,
                _ => ShapeProperty(name),
            },

            HostNodeType.Polyline => name switch
            {
                HostProp.Points => TransformedPolyline.PointsProperty,
                HostProp.FillRule => TransformedPolyline.FillRuleProperty,
                _ => ShapeProperty(name),
            },
    ...
    private static BindableProperty? ShapeProperty(HostProp name)
    {
        return name switch
        {
            ...
            HostProp.RenderTransform => ShapeTransform.GeometryTransformProperty,
            _ => null,
        };
    }
}
