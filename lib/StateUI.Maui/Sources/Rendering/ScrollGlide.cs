...
internal static class ScrollGlide
{
    ...
    internal static double Held(double to, double from, double interval, double origin, int most)
    {
        ...
        double started = Math.Round((from - origin) / interval, MidpointRounding.AwayFromZero);
        double wanted = Math.Round((to - origin) / interval, MidpointRounding.AwayFromZero);
        ...
        double going, double aim, double at, double interval, double origin, double least)
    {
        ...
        double here = origin + (Math.Round(
            (aim - origin) / interval,
            MidpointRounding.AwayFromZero) * interval);
        double rounded = origin + (Math.Round(
            (going - origin) / interval,
            MidpointRounding.AwayFromZero) * interval);
    ...
    internal static int Cells(double from, double to, double interval, double origin)
    {
        ...
        double here = Math.Round((from - origin) / interval, MidpointRounding.AwayFromZero);
        double there = Math.Round((to - origin) / interval, MidpointRounding.AwayFromZero);
