...
internal sealed class SwiftDrawable(HostValue[] commands) : IDrawable
{
    ...
    private static void Run(ICanvas canvas, Kind kind, HostValue[] record)
    {
        ...
        switch (kind)
        {
            ...
            case Kind.TextColor when Colour(record, 1) is Color colour:
                canvas.FontColor = colour;
                break;

            case Kind.StrokeWidth when Number(record, 1) is double size:
            ...
            // captions not.
            case Kind.DrawText when Numbers(record, 4) is [double x, double y, double width, double height]
