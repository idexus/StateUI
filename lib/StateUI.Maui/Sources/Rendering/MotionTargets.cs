...
internal sealed class MotionFrame : IMotionTarget
{
    ...
    /// <remarks>
    /// <para>
    /// A PLACE IS THE CHILD'S FRAME, and arranging a view hands it a SLOT: the
    /// frame with the margin around it, out of which the view takes its margin
    /// again. <see cref="Read"/> reads the frame, so the margin goes back around
    /// it here. Handed the frame itself, a view takes its margin out twice - a
    /// button with a margin of 20, walked to a new width, stands at a height of
    /// nothing.
    /// </para>
    /// <para>
    ...
    {
        Rect frame = (Rect)Compose(from);
        Thickness margin = _child.Margin;
