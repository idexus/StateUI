...
public class WireEnumTests
{
    ...
    private static readonly Dictionary<string, string> StaysOnThisSide = new()
    {
        ...
        // WHICH fields of a journey a same-process native host is reporting
        // back to the core. This is a typed Swift call, not wire vocabulary.
        ["HostJourneyUpdate"] = "used only by the typed native-host SPI",
    };
    ...
    public void TheSwiftSourcesAreActuallyBeingRead()
    {
        ...
        Assert.Contains("LineBreak", all.Keys);
        Assert.Equal(4, all["LineBreak"].Single(member => member.Member == "tailTruncation").Value);
        Assert.Contains("BorderShape.Kind", all.Keys);
    }
...
internal static class SwiftEnums
{
    ...
    /// <summary>`lib/StateUI/Sources`, found by walking up from the test assembly.</summary>
    internal static string Sources
    {
        get
        {
            ...
            {
                string candidate = Path.Combine(directory.FullName, "lib", "StateUI", "Sources");
            ...
            throw new DirectoryNotFoundException(
                "lib/StateUI/Sources was not found above " + AppContext.BaseDirectory);
        }
    }
