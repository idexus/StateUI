...
struct PlacedSample: SampleContent {
    ...
    @State(asks: .never) private var scrolled = Double(PlacedSample.cards.count / 2)
        * PlacedSample.reach
    ...
    @State(asks: .never) private var dots = PlacedRun()
    ...
    @State(asks: .never) private var dotRoom = Rect(0, 0, 0, 0)
    ...
    static let code = """
        ...
        @State(asks: .never) private var scrolled = 270.0
        @State(asks: .never) private var dragged = 0.0
    ...
    var notes: Element? {
        VStack {
            ...
            Label("Nothing here is state - not the two numbers, not the room, "
                + "and not where a single card goes. A state the host holds - "
                + "`@State(asks: .never)` - is read and "
