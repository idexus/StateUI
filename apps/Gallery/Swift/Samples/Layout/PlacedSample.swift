...
struct PlacedSample: SampleContent {
    ...
    @Hosted private var scrolled = Double(PlacedSample.cards.count / 2)
        * PlacedSample.reach
    ...
    @Hosted private var dots = PlacedRun()
    ...
    @Hosted private var dotRoom = Rect(0, 0, 0, 0)
    ...
    static let code = """
        ...
        @Hosted private var scrolled = 270.0
        @Hosted private var dragged = 0.0
    ...
    var notes: Element? {
        VStack {
            ...
            Label("Nothing here is state - not the two numbers, not the room, "
                + "and not where a single card goes. A `@Hosted` is read and "
